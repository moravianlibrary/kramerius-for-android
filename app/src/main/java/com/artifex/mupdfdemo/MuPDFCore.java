...
public class MuPDFCore
{
	...
	public MuPDFCore(Context context, String filename) throws Exception
	{
		...
		if (globals == 0)
		{
			throw new Exception(String.format(context.getString(R.string.mupdf_cannot_open_file_Path), filename));
		}
	...
	public MuPDFCore(Context context, byte buffer[], String magic) throws Exception {
		...
		if (globals == 0)
		{
			throw new Exception(context.getString(R.string.mupdf_cannot_open_buffer));
		}
		file_format = fileFormatInternal();
		isUnencryptedPDF = isUnencryptedPDFInternal();
